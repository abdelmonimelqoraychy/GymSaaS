...
function AdminLogin() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!isAdmin(user)) {
        clearSession();
        setError("Ce compte est un compte adhérent et n’a pas accès à l’administration.");
        return;
      }
  ...
  return (
    <main className="admin-login-page">
      <Link className="brand admin-login-brand" to="/">
        GYM<span>SAAS</span>
      </Link>
      ...
      <form className="admin-login-card" onSubmit={handleSubmit}>
        ...
        <h1>Accès Admin</h1>
        <p>Réservé à l’équipe de gestion GymSaaS.</p>
